...
module hole_bushing(stiff, lenght=11){
	difference(){
		union(){
			translate(v=[-8,-8,0])cube(size = [16,8,lenght]);
			cylinder(h = lenght, r=8);
		}
	//cutout inside
	//translate(v=[0,0,-1]) cylinder(h = lenght+2, r=5);

	//HACK for Slic3r making holes too small
	translate(v=[0,0,-1]) cylinder(h = lenght+2, r=5.3);
	}
}
...
module hole_bushing_mount(){
difference(){
	translate(v=[0,5,8])rotate(a=[90,0,0]){
		hole_bushing(false, 10);
		translate(v=[-7,-13,0])cube(size = [14,5,10]);
		//translate(v=[-2,-13,-3])cube(size = [4,5,16]);
		//translate(v=[-2,-12,-3])cube(size = [4,7.5,2]);
		//translate(v=[-2,-12,11])cube(size = [4,7.5,2]);
...
module z_pressfit_bushings(){
	// STUFF FOR X-ENDS
	
	difference(){
		// Slider
		union(){
			translate([0,6.5,7.9]) cube([23,17,15.8],center=true);
			translate([0,9.5,0]) cylinder(h = 60, r=9);
		}
		// circular cutouts for press-fit bushings
		//translate([0,10,32.5]) cube([17,17,70],center=true);
		translate([0,9.5,0]) cylinder(h = 60+2, r=5);
		
		//HACK for Slic3r making holes too small
		//translate([0,9.5,0]) cylinder(h = 60+2, r=5.4);
	}
}
